...
function Signup() {
    ...
    const handleSignup = (event) => {
        ...
        && (/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(password))) { //regex : le mot de passe doit contenir au moins 8 caractères, 1 lettre, 1 chiffre et 1 caractère spécial
            ...
            let signupFormData = { name : name, firstName : firstName, email : email, password: password, isAdmin : 0 };
    ...
    return (
        ...
        <form id ='signupForm' className="logform" onSubmit={handleSignup}>
            <label>Nom
                <input id='name' className="input" type="string" name="name" maxLength="40" value={name} onChange={e => setName(e.target.value)} required/>
            </label>
            <label>Prénom
                <input id='firstName' className="input" type="sring" name="firstName" maxLength="40" value={firstName} onChange={e => setFirstName(e.target.value)} required />
            </label>   
            <label>email
                <input id='email' className="input" type="email" name="email" value={email} maxLength="40" autoComplete="username" onChange={e => setEmail(e.target.value)} required />
            </label>
            <label>mot de passe
                <input id='password' className="input" type="password" name="password" value={password} maxLength="40" autoComplete="current-password" onChange={e => setPassword(e.target.value)} required />
            </label>
