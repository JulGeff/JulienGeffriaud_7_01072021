                  ...
                  <div >
                    ...
                    <div key={i} className = "forum__displayposts__content">
                        <Link to={"/forum/publication?id=" + item.id}  className = "forum__displayposts__content__link">
                            <div>
                                <h2>{item.title}</h2>
                                <p className='forum__displayposts__content__link__subtitle'> Publié par <strong>{item.user.firstName} {item.user.lastName}</strong> le {item.updatedAt.substring(8,10).padStart(2, '0')}/{item.updatedAt.substring(5,7).padStart(2, '0')}/{item.updatedAt.substring(0,4)} à {item.updatedAt.substring(11,16)}</p>
                                <p className='forum__displayposts__content__link__text'>{item.content}</p>
                            </div>                     
                        </Link>
                        <div className='forum__displayposts__content__link__buttons'>

                        {/* si user connecté est l'auteur de la publication, on affiche un lien pour l'éditer*/}    
                        {item.userId===user.id
                          ? (<Link to={"/forum/editpublication?id=" + item.id}  className = "forum__displayposts__content__link__buttons__edit">
                            <div aria-label="Modifier publication">
                         ...
                         {item.userId===user.id || user.isAdmin   
                          ? (
                            <div aria-label="Supprimer publication"  tabindex="0" className = "forum__displayposts__content__link__buttons__delete" onClick = {e => {if (window.confirm('Etes-vous sûr·e de vouloir supprimer cette publication ?')) handleDelete(e, item.id)}} >       
