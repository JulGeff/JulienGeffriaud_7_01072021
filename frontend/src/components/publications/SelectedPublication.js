      ...
      <p className='publication__subtitle'>Publié par <strong>{selectedPublication.user.firstName} {selectedPublication.user.lastName}</strong> le {selectedPublication.updatedAt.substring(8,10).padStart(2, '0')}/{selectedPublication.updatedAt.substring(5,7).padStart(2, '0')}/{selectedPublication.updatedAt.substring(0,4)} à {selectedPublication.updatedAt.substring(11,16)}</p>
        ...
        <div className = 'publication__icons'>
          ...
          {selectedPublication.userId===user.id
                  ? (<Link aria-label="Modifier publication" to={"/forum/editpublication?id=" + selectedPublication.id} className = "publication__icons__edit">
          ...
          {selectedPublication.userId===user.id || user.isAdmin
                  ? (<div aria-label="Supprimer publication" tabindex="0" className = "publication__icons__delete" onClick = {e => {if (window.confirm('Etes-vous sûr·e de vouloir supprimer cette publication ?')) handleDelete(e, selectedPublication.id)}} >
            ...
            <div className="publication__displaycomments__list__info" >
              ...
              {/* si user connecté est l'auteur du commentaire ou admin, on affiche un lien pour le supprimer*/} 
               {item.userId===user.id || user.isAdmin
                ? (<div aria-label="Supprimer commentaire" tabindex="0" onClick = {e => {if (window.confirm('Etes-vous sûr·e de vouloir supprimer ce commentaire ?')) handleDeleteComment(e, item.id)}} >
